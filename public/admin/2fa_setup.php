...
$s->execute([':id'=>$_SESSION['user_id']]); $u=$s->fetch(); if(!$u){ http_response_code(404); die('Utente non trovato'); }
...
$issuer=rawurlencode('ARENA'); $account=rawurlencode($u['username']);
...
?>
<!doctype html><html lang="it"><head><meta charset="utf-8"><title>Setup 2FA</title>
<link rel="stylesheet" href="/assets/base.css"><link rel="stylesheet" href="/assets/header_admin.css">
<style>.wrap{max-width:520px;margin:30px auto;padding:0 20px;color:#fff}.card{background:#111;border:1px solid #222;border-radius:12px;padding:16px}
