...
// Se round non passato, prendo quello corrente
if ($round <= 0) {
...
// Mappa etichette
$labels = [
  'pending'=>'—', 'home_win'=>'1 (Casa vince)', 'draw'=>'X (Pareggio)',
  'away_win'=>'2 (Trasferta vince)', 'postponed'=>'Rinviata', 'void'=>'Annullata'
];
?>
<!doctype html>
<html lang="it">
<head>
  ...
  <style>
    ...
    table{width:100%;border-collapse:collapse} th,td{padding:8px;border-bottom:1px solid rgba(255,255,255,.1)}
    ...
    select, input[type=datetime-local]{height:32px;border:1px solid rgba(255,255,255,.25);border-radius:8px;background:#0a0a0b;color:#fff;padding:0 8px}
  </style>
</head>
<body>
...
<div class="wrap">
  ...
  <div class="card">
    <?php if (!$rows): ?>
      <div style="color:#aaa">Nessun evento per questo round.</div>
    <?php else: ?>
      <table>
        <thead>
          <tr>
        ...
        <tbody>
          <?php foreach ($rows as $e): $formId = 'f_'.$e['id']; ?>
            <tr>
