...
// Restituisce i movimenti crediti dell'utente loggato.
// Output: { ok:true, items:[{id, ts, ts_h, type, type_label, amount, sign, tournament_code, tournament_name}] }
...
try {
  ...
  while ($r = $q->fetch(PDO::FETCH_ASSOC)) {
    ...
    $amt = (int)$r['amount']; // nei tuoi flussi: addebiti negativi, accrediti positivi
    $sign = $amt >= 0 ? 'in' : 'out';
  ...
  out(['ok'=>true, 'items'=>$items, 'page'=>$page, 'limit'=>$limit]);
